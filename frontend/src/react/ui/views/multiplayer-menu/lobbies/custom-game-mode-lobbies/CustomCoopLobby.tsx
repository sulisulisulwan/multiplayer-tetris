...
import { CoopSlotsCustom, PartyState, UserDataFromAPI, UserDataFromDB } from 'multiplayer-tetris-types'
            ...
            >
              {
                team1WithData.map((player, idx) => {
                  return (
                    <div key={player ? player.id : idx} style={{
                      paddingLeft: 10,
                      paddingRight: 10
                    }}>
                      <PlayerSlot
                        key={player ? player.name + player?.id + '' + idx : idx}
                        slotId={`team1-${idx}`}
