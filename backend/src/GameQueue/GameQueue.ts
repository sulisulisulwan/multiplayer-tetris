import { GameRoomDataAPI, OnePlayerLocalGameState, PartyId, PartyRoomDataAPI, RoomId, SocketDataItem } from "multiplayer-tetris-types"
...
class OneVAllQueue extends GameQueue {
  ...
  protected async createGame(party1: PartyRoomDataAPI, party2: PartyRoomDataAPI) {
    ...
    const gameRoomId = await this.db.createMultiplayerGameRoom(party1.gameType, party1, party2)
...
class OneVOneQueue extends GameQueue {
  ...
  protected async createGame(party1: PartyRoomDataAPI, party2: PartyRoomDataAPI) {
    ...
    const gameRoomId = await this.db.createMultiplayerGameRoom(party1.gameType, party1, party2)
    const gameRoomData = await this.db.getRoomByIdAndType(gameRoomId, 'game') as GameRoomDataAPI

    gameRoomData.users.forEach((userData: UserDataFromDB) => {
      this.parentQueueHandler.getServer().getSocketByUserId(userData.id).join(gameRoomData.id)
    })

    const gameRoomAllData = {
      ...gameRoomData,
      currState: gameRoomData.users.reduce((currGameStates, userData) => {
        currGameStates[userData.id] = {
          type: 'multiplayer',
          gameMode: gameRoomData.gameType,
      ...
      }, {} as Record<UserId, OnePlayerLocalGameState>)
    }

    this.parentQueueHandler.broadcastTo(gameRoomData.id, {
      action: 'matchFound',
      data: {
        gameRoomData: gameRoomAllData
      } 
    })
  }
